...
class RestroomsViewController: UITableViewController, UISearchControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        navigationItem.title = "Restrooms"
    }
    ...
    func getLocations(values : [String]){
        ...
        for i in values{
            if !locations.contains(i){
                ...
                // gets only the locations containing the word "Restroom"
                locations = locations.filter{ $0.contains("Restroom") }
            }
        }
...
extension RestroomsViewController: UISearchResultsUpdating {
